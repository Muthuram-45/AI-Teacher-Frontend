...
export default function AttendanceSidebar({ attendance, doubtsWithAnswers = [], classSummary, topic, onClose, right = 0 }) {
    ...
    const downloadExcel = () => {
        ...
        studentAttendance.forEach(s => {
            ...
            rows.push([s.identity, firstJoin, stayTime, s.joinCount, questionText]);
        });
        ...
        // 4. Set Merges
        ws['!merges'] = [
            { s: { r: 0, c: 0 }, e: { r: 0, c: 4 } }, // Merge "CLASS DETAILS" (Row 1)
            { s: { r: 5, c: 0 }, e: { r: 5, c: 4 } } // Merge "STUDENT DETAILS" (Row 6)
        ];
